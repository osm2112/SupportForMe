  ...
  CREATE OR REPLACE TRIGGER DELETE_MEMBER_TRIGGER before update on member
 for each row
   BEGIN
   if updating then :NEW.password := null;
    INSERT into delete_member(user_id, password, name, address, phone_num, email, introduction, postcode, addr_detail, ip_address) 
    values ( :old.user_id, :old.password, :old.name, :old.address, :old.phone_num, :old.email, :old.introduction, :old.postcode, :old.addr_detail,SYS_CONTEXT('userenv', 'ip_address'));
    END if;
   END;
/
ALTER TRIGGER DELETE_MEMBER_TRIGGER ENABLE;
